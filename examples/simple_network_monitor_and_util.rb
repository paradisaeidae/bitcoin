...
def on_ping(nonce)
 send_data(Bitcoin::Protocol.pong_pkt(nonce)) if nonce end

def on_reject(reject)
 log.info { "reject #{reject}" } end
...
def on_tx(tx)
 ...
 if tx.hash == @ask_tx
  @args[:result] = tx
  @args[:callback] ? (close_connection; @args[:callback].call(tx)) : EM.stop end end
...
def on_block(block)
 ...
 if block.hash == @ask_block
  if @ask_tx
   if tx = block.tx.find{|tx| tx.hash == @ask_tx } then on_tx(tx)
   else
    log.info { "@ask_tx #{@ask_tx} not in @ask_block #{@ask_block}" }
    @args[:result] = nil
    @args[:callback] ? (close_connection; @args[:callback].call(nil)) : EM.stop end
  else
   @args[:result] = block
   @args[:callback] ? (close_connection; @args[:callback].call(block)) : EM.stop end end end
...
def on_inv_transaction(hash)
 ...
 send_data Bitcoin::Protocol.getdata_pkt(:tx, [hash]) end

def on_inv_block(hash)
 log.info { "peer told us about block: #{hth(hash)}" }
 log.info { "asking peer for block: #{hth(hash)}" }
 send_data Bitcoin::Protocol.getdata_pkt(:block, [hash]) end
...
def initialize(host, port, node=nil, opts={})
 ...
 @args = opts
 @ask_tx, @ask_block, @send_tx = opts.values_at(:ask_tx, :ask_block, :send_tx) end

def receive_data(data); @parser.parse(data)  end
def post_init
 log.info { "peer connected" }
 on_handshake_begin end
def unbind; log.info { "peer disconnected" } end
def set_host(host, port=8333)
 @host, @port = host, port end
...
def log
 ...
 @log = Bitcoin::Logger::LogWrapper.new("#@host:#@port", @logger || @node.log) end

def hth(h); h.unpack("H*")[0]; end
def htb(h); [h].pack("H*"); end
def self.connect(host, port, *args)
 EM.connect(host, port, self, host, port, *args) end
...
def self.connect_known_nodes(count=1)
 connect_random_from_dns(Bitcoin.network[:known_nodes], count) end end end
...
args = {
 ...
 callback:  proc{|i|
  ...
  when Bitcoin::Protocol::Tx
   ...
   File.open("tx-#{i.hash}.json", 'wb'){|f| f.print i.to_json } end
  EM.stop } }
  
EM.run do
 if args[:set_project]
   Bitcoin.network = args[:set_project].to_sym
   p Bitcoin.network_project  end

 if args[:use_node] then SimpleNode::Connection.connect_random_from_dns([args[:use_node]], 1, nil, args)
 else SimpleNode::Connection.connect_random_from_dns([], 1, nil, args) end end end
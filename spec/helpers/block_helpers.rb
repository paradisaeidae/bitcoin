...
def create_block(prev, store = true, transaction = [], key = Bitcoin::Key.generate, coinbase_value = 50e8, opts = {})
 @store ||= nil
 ...
 block = build_block(Bitcoin.decode_compact_bits(opts[:bits])) do |b|
  ...
  b.tx do |t|
   ...
   t.output do |o|
    ...
    o.script { |s| s.recipient key.addr } end end
  transaction.each do |cb| b.tx { |t| cb.call(t) } end end

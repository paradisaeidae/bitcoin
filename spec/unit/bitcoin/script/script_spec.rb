...
describe Bitcoin::Script do
  SCRIPT = [
    ...
    '6a04deadbeef' ].map { |s| [s].pack('H*') }.freeze # OP_RETURN deadbeef
  ...
  describe 'serialization' do
    it '#to_string' do
      ...
      expect(Bitcoin::Script.from_string('ff' * (Bitcoin::Script::OP_PUSHDATA1 - 1))
          .to_payload[0]
      ).to eq([Bitcoin::Script::OP_PUSHDATA1 - 1].pack('C*'))
      expect(Bitcoin::Script.from_string('ff' * Bitcoin::Script::OP_PUSHDATA1)
          .to_payload[0..1]
      ...
      # data below taken from the outputs of mainnet (Still valid on BSV) tx:
      ...
      [ ["\x01", '238:1:01', true],
        ...
        expect(Bitcoin::Script.new(payload).instance_eval { @parse_invalid })
          .to eq(parse_invalid)
        expect(Bitcoin::Script.from_string(string).to_payload).to eq(payload) end
      ...
      # data below found in transactions in testnet3 block (Exists on BSV testnet though void of transactions)
    ...
    it 'Script#binary_from_string' do
      ...
      expect(Bitcoin::Script
          ...
          .to_string).to eq('0 ffff OP_CODESEPARATOR 1 ffff 1 OP_CHECKMULTISIG')

      [1, 2, 4].each do |n|
        script = "OP_PUSHDATA#{n} 01 ff"
        expect(Bitcoin::Script.binary_from_string(script))
          .to eq(Bitcoin::Script.binary_from_string(
                   Bitcoin::Script.from_string(script).to_string )) end
  ...
  describe 'get keys/addresses' do
    ...
    it '#get_pubkey_address' do
      expect(Bitcoin::Script.new(SCRIPT[0]).get_pubkey_address)
        .to eq('12cbQLTFMXRnSzktFkuoG3eHoMeFtpTu3S') end
    ...
    it '#get_multisig_pubkeys' do
      ...
      expect(
        Bitcoin::Script
          ...
          .get_multisig_pubkeys ).to eq(expected) end
    ...
    it '#get_multisig_addresses' do
      ...
      ).to eq(['1NdB761LmTmrJixxp93nz7pEiCx5cKPW44', nil]) end
    ...
    it '#get_address' do
      ...
      expect(Bitcoin::Script.new(SCRIPT[8]).get_address)
        .to eq('bc1qt5d4dd3aw98whe2zxz2jtayykl5ada5xkdupkmmpa7f96ekk76sqvmrunq') end
    ...
    it '#get_addresses' do
      ...
      expect(Bitcoin::Script.new(SCRIPT[7]).get_addresses)
        .to eq(['bc1qrcs9z5wfpstyw5mr6ydhhrprtnmv0454y6laym']) end
  ...
  describe 'determine type' do
    it '#is_standard?' do
      ...
      expect(Bitcoin::Script.new(SCRIPT[8]).is_standard?).to be true end
    ...
    it '#is_hash160?' do
      ...
      expect(Bitcoin::Script.new(SCRIPT[8]).is_hash160?).to be false end
    ...
    it '#is_op_return?' do
      ...
      expect(Bitcoin::Script.from_string('OP_RETURN deadbeef').is_op_return?)
        .to be true
      expect(Bitcoin::Script.from_string('OP_RETURN OP_CHECKSIG').is_op_return?).to be true end
  ...
  describe 'generate scripts' do
    it 'should generate pubkey script' do
      ...
      expect(Bitcoin::Script.to_pubkey_script(PUBKEYS[1]))
        .to eq(Bitcoin::Script.from_string("#{PUBKEYS[1]} OP_CHECKSIG").raw) end
    ...
    it 'should generate multisig script' do
      expect(Bitcoin::Script.to_multisig_script(2, *PUBKEYS[0..2]))
        .to eq(
          Bitcoin::Script.from_string(
            "2 #{PUBKEYS[0..2].join(' ')} 3 OP_CHECKMULTISIG" ).raw )
      expect(Bitcoin::Script.to_multisig_script(1, *PUBKEYS[0..1]))
        .to eq(
          Bitcoin::Script.from_string(
            "1 #{PUBKEYS[0..1].join(' ')} 2 OP_CHECKMULTISIG" ).raw )

      m = n = 16
      expect(
        Bitcoin::Script.new(
          Bitcoin::Script.to_multisig_script(m, *(['a'] * n))).to_string
    ...
    it 'should generate op_return script' do
      ...
      expect(Bitcoin::Script.to_op_return_script)
        .to eq(Bitcoin::Script.from_string('OP_RETURN').raw) end
    ...
    it 'should determine type for address script' do
      ...
      expect(Bitcoin::Script.to_address_script(address))
        .to eq(
          Bitcoin::Script.from_string(
            "OP_DUP OP_HASH160 #{hash160} OP_EQUALVERIFY OP_CHECKSIG" ).raw ) end
  ...
  describe 'generate script sigs' do
    ...
    it 'should reject an improperly encoding public key' do
      ...
      end.to raise_error(RuntimeError, 'pubkey is not in binary form') end
  ...
  it '#run' do
    ...
    script = Bitcoin::Script.new(['0063bac0d0e0f0f1f2f3f3f4ff675168'].pack('H*'))
    ...
    expect(script.run).to be true end
  ...
  it 'should debug script branches (OP_IF/NOTIF/ELSE/ENDIF) correctly' do
    ...
    expect(script.debug).to eq(expected) end
  ...
  describe 'Implements BIP62' do
    it 'tests for incorrectly encoded S-values in signatures' do
      ...
      expect(Bitcoin::Script.is_low_der_signature?(sig.pack('C*'))).to be false end
  ...
  describe 'Implements BIP66' do
    ...
    let(:p1) do
      '038282263212c609d9ea2a6e3e172de238d8c39cabd5ac1ca10646e23fd5f51508' end
    let(:p2) do
      '03363d90d447b00c9c99ceac05b6262ee053441c7e55552ffe526bad8f83ff4640' end
    ...
    def build_crediting_tx(script_pk)
      ...
      tx.add_in(input)
      tx.add_out(output)
      Bitcoin::Protocol::Tx.new(tx.to_payload) end

    def build_spending_tx(script_sig, tx_credit)
      tx = Bitcoin::Protocol::Tx.new
      input = Bitcoin::Protocol::TxIn.new( tx_credit.binary_hash, 0, 2, script_sig )
    ...
    def run_script_test(script_sig_str, script_pk_str, opts = {})
      ...
      tx.verify_input_signature(0, tx_credit, Time.now.to_i, opts) end
    ...
    it 'overly long signature fails with DERSIG passes without' do
      ...
      expect(run_script_test(script_sig, script_pk)).to be true end

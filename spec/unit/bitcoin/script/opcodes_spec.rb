...
describe Bitcoin::Script do
  ...
  module Bitcoin
   ...
   class Script
    attr_writer :stack
    attr_accessor :stack_alt end end
  ...
  it 'should do OP_NOP' do
   expect(op(:nop, ['foobar'])).to eq(['foobar']) end

  it 'should do OP_DUP' do
   expect(op(:dup, ['foobar'])).to eq(%w[foobar foobar]) end

  it 'should do OP_SHA256' do
   expect(op(:sha256, ['foobar']))
    .to eq( [['c3ab8ff13720e8ad9047dd39466b3c8974e592c2fa383d4a3960714caef0c4f2'].pack('H*')] ) end

  it 'should do OP_SHA1' do
   expect(op(:sha1, ['foobar']))
    .to eq([['8843d7f92416211de9ebb963ff4ce28125932878'].pack('H*')]) end

  it 'should do OP_HASH160' do
   expect(op(:hash160, ['foobar']))
    .to eq([['f6c97547d73156abb300ae059905c4acaadd09dd'].pack('H*')]) end

  it 'should do OP_RIPEMD160' do
   expect(op(:ripemd160, ['foobar']))
    .to eq([['a06e327ea7388c18e4740e350ed4e60f2e04fc41'].pack('H*')]) end

  it 'should do OP_HASH256' do
   expect(op(:hash256, ['foobar']))
    .to eq( [['3f2c7ccae98af81e44c0ec419659f50d8b7d48c681e5d57fc747d0461e42dda1'].pack('H*')] ) end

  it 'should do OP_TOALTSTACK' do
   expect(op(:toaltstack, ['foobar'])).to be_empty
   expect(script.stack_alt).to eq(['foobar']) end
  ...
  it 'should do OP_0' do
   expect(op('0', ['foo'])).to eq(['foo', '']) end

  it 'should do OP_1' do
   expect(op('1', ['foo'])).to eq(['foo', 1]) end
  ...
  it 'should do OP_MIN' do
   [ [[4, 5], 4],
     ...
     expect(op(:min, stack)).to eq([expected]) end end
  ...
  it 'should do OP_MAX' do
   [ [[4, 5], 5],
     ...
     expect(op(:max, stack)).to eq([expected]) end end

  it 'should do op_2over' do
   expect(op('2over', [1, 2, 3, 4])).to eq([1, 2, 3, 4, 1, 2]) end

  it 'should do op_2swap' do
   expect(op('2swap', [1, 2, 3, 4])).to eq([3, 4, 1, 2]) end

  it 'should do op_ifdup' do
   expect(op(:ifdup, [1])).to eq([1, 1])
   expect(op(:ifdup, ['a'])).to eq(%w[a a])
   expect(op(:ifdup, [0])).to eq([0]) end

  it 'should do op_1negate' do
   expect(op('1negate', [])).to eq([-1]) end

  it 'should do op_depth' do
   expect(op(:depth, [])).to eq([0])
   expect(op(:depth, [1, 2, 3])).to eq([1, 2, 3, 3]) end
  ...
  it 'should do op_boolor' do
   [ [[1, 1], 1],
    ...
    expect(op(:boolor, stack)).to eq([expected]) end end
  ...
  it 'should do op_greaterthanorequal' do
    [
      ...
      expect(op(:greaterthanorequal, stack)).to eq([expected]) end end
  ...
  it 'should do op_within' do
    [ [[0, 0, 1], 1],
      ...
      expect(op(:within, stack)).to eq([expected]) end end

  it 'should do op_numequal' do
    [ [[0, 0], 1],
      [[0, 1], 0]
    ].each do |stack, expected|
      expect(op(:numequal, stack)).to eq([expected]) end end

  it 'should do op_numequalverify' do
    [ [[0, 0], []],
      [[0, 1], [0]]
    ].each do |stack, expected|
      expect(op(:numequalverify, stack)).to eq(expected) end end
  ...
  it 'should do op_over' do
    [
      ...
      expect(op(:over, stack)).to eq(expected) end end
  ...
  it 'should do op_pick' do
    ...
    [
      ...
      expect(op(:pick, stack)).to eq(expected)
      expect(script).to be_invalid end end
  ...
  it 'should do op_size' do
    [ [[0], [0, 0]],
      ...
      expect(op(:size, stack)).to eq(expected) end end
  ...
  it 'should do if/notif/else/end' do
    [ '1 1 OP_IF OP_IF 1 OP_ELSE 0 OP_ENDIF OP_ENDIF',
      ...
      expect(Bitcoin::Script.from_string(script).run).to be true end end
  ...
  it 'should do OP_CHECKSIG' do
    ...
    pubkey = [
      ...
      'e31f8be90c385e27883a9d82780283d19507d7fa2e1e71a1d11bc3a52caf3' ].pack('H*')
    signature = [
    ...
    verify_callback = proc do |pub, sig, hash_t|
      ...
      hash = signature_data
      Bitcoin.verify_signature(hash, sig, pub.unpack('H*')[0]) end
    expect(script.op_checksig(verify_callback)).to eq([1])
    ...
    verify_callback = proc do |pub, _, _|
      hash = signature_data
      Bitcoin.verify_signature(hash, 'foo', pub.unpack('H*')[0]) end
    expect(script.op_checksig(verify_callback)).to eq([0])

    script.stack = [signature + hash_type, pubkey]
    verify_callback = proc do |_, sig, _|
      hash = signature_data
      Bitcoin.verify_signature(hash, sig, 'foo') end
    expect(script.op_checksig(verify_callback)).to eq([0])
    ...
    result = script.run do |pk, sig, _|
      k = Bitcoin::Key.new nil, pk.unpack('H*')[0]
      k.verify('foobar', sig) end

    expect(result).to be true
    expect(script.stack).to be_empty end
  ...
  it 'should do OP_CHECKMULTISIG' do
    ...
    expect(run_script(script, 'foobar')).to be false end

  it 'should not do P2SH' do
    k1 = Bitcoin::Key.new
    k1.generate
    ...
    expect(script.is_p2sh?).to be true
    expect(run_script(script.to_string, 'foobar')).to be true end
  ...
  it 'should skip OP_EVAL' do
    ...
    expect(Bitcoin::Script.from_string('0 OP_EVAL').run).to be false end
 ...
 it 'should do testnet3 scripts' do
  [ 'OP_1NEGATE OP_1NEGATE OP_ADD 82 OP_EQUAL',
   ...
   expect(result).to be true end
  ['ffffff7f ffffff7f OP_ADD ffffff7f OP_ADD OP_TRUE'
  ].each do |script|
   parsed_script = Bitcoin::Script.from_string(script)
   result = parsed_script.run
   expect(result).to be false end end
 ...
 it 'should do OP_VER' do
  ...
  expect(s).not_to be_invalid end
 ...
 it 'should not allow DISABLED_OPCODES' do
  Bitcoin::Script::DISABLED_OPCODES.each do |opcode|
   ...
   expect(s.run).to be false
   expect(s).to be_invalid end end
 ...
 it 'should do OP_CHECKSIGVERIFY and OP_CHECKMULTISIGVERIFY' do
  ...
  expect(tx2.verify_input_signature(0, tx1)).to be true end end
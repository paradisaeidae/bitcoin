...
describe Bitcoin do
  ...
  describe '.hash160_from_address' do
    ...
    context 'testnet' do
      ...
      it 'produces the expected hash' do
        ...
        ).to eq('62e907b15cbf27d5425399ebf6f0fb50ebb88f18') end

      it 'returns nil for invalid address' do
        expect(Bitcoin.hash160_from_address('bad-address-testnet')).to be_nil end
      ...
      it 'survives rounds of conversion from hash160 to address' do
        ...
        expect(
          Bitcoin.hash160_from_address(
            Bitcoin.hash160_to_address(pubkey_hash_hex) )).to eq(pubkey_hash_hex)

        expect(
          Bitcoin.hash160_to_address(
            Bitcoin.hash160_from_address(addr) )).to eq(addr) end end
    ...
    context 'mainnet' do
      ...
      it 'returns expected values for addresses' do
        ...
        ).to eq('751e76e8199196d454941c45d1b3a323f1433bd6') end

      it 'returns nil for invalid address' do
        expect(Bitcoin.hash160_from_address('bad-address-mainnet')).to be_nil end
      ...
      it 'survives rounds of conversion from hash160 to address' do
        ...
        expect(
          Bitcoin.hash160_to_address(
            Bitcoin.hash160_from_address(addr) ) ).to eq(addr) end end end
  ...
  describe '.address_checksum?' do
    context 'testnet' do
      ...
      it 'returns true for valid addresses' do
        expect(
          Bitcoin.address_checksum?('mpXwg4jMtRhuSpVq4xS3HFHmCmWp9NyGKt') ).to be true
        expect(
          Bitcoin.address_checksum?('1D3KpY5kXnYhTbdCbZ9kXb2ZY7ZapD85cW') ).to be true end

      it 'returns false for invalid addresses' do
        expect(Bitcoin.address_checksum?('f0f0f0')).to be false end end
    ...
    context 'mainnet' do
      ...
      it 'returns true for valid addresses' do
        expect(
          Bitcoin.address_checksum?('1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa') ).to be true
        expect(
          Bitcoin.address_checksum?('mpXwg4jMtRhuSpVq4xS3HFHmCmWp9NyGKt') ).to be true end

      it 'returns false for invalid addresses' do
        expect(Bitcoin.address_checksum?('f0f0f0')).to be false end end end
  ...
  describe '.valid_address?' do
    context 'testnet' do
      ...
      it 'is true for valid addresses' do
        expect(
          Bitcoin.valid_address?('mpXwg4jMtRhuSpVq4xS3HFHmCmWp9NyGKt') ).to be true end

      it 'is false for invalid addresses' do
        expect(
          Bitcoin.valid_address?('1D3KpY5kXnYhTbdCbZ9kXb2ZY7ZapD85cW') ).to be false
        expect(
          Bitcoin.valid_address?('1moYFpRM4LkTV4Ho5eCxiEPB2bSm3AsJNGj')).to be false
        expect(Bitcoin.valid_address?('f0f0f0')).to be false end
      ...
      it 'successfully validates a series of new addresses' do
        400.times do
          ...
          expect(Bitcoin.valid_address?(addr[0])).to be true end end end
    ...
    context 'mainnet' do
      ...
      it 'is true for valid addresses' do
        expect(
          Bitcoin.valid_address?('1D3KpY5kXnYhTbdCbZ9kXb2ZY7ZapD85cW')).to be true
        expect(
          Bitcoin.valid_address?('1ZQxJYBRmbb2rDNYPhd96x3eMbNnPD98q')).to be true
        expect(
          Bitcoin.valid_address?('12KhCL8nGK3Luy7ehU3AxPs1mTocdessLM')).to be true
        expect(
          Bitcoin.valid_address?('1AnNQgfaGgSKejzR6km74tyQPDGwZBBVT')).to be true end

      it 'is false for invalid addresses' do
        expect(
          Bitcoin.valid_address?('mpXwg4jMtRhuSpVq4xS3HFHmCmWp9NyGKt')).to be false
        expect(
          Bitcoin.valid_address?('2D3KpY5kXnYhTbdCbZ9kXb2ZY7ZapD85cW')).to be false
        expect(
          Bitcoin.valid_address?('1D3KpY5kXnYhTbdCbZ9kXb2ZY7ZapD85cX')).to be false
        expect(
          Bitcoin.valid_address?('1moYFpRM4LkTV4Ho5eCxiEPB2bSm3AsJNGj')).to be false
        expect(
          Bitcoin.valid_address?('f0f0f0')).to be false end
      ...
      it 'successfully validates a series of new addresses' do
        400.times do
          ...
          expect(Bitcoin.hash160_from_address(addr[0])).to eq(addr[-1])
          expect(Bitcoin.hash160_to_address(addr[-1])).to eq(addr[0])
          expect(Bitcoin.valid_address?(addr[0])).to be true end end end end
  ...
  describe '.base58_to_int' do
    it 'returns the expected integer equivalent' do
      expect(
        ...
        15_016_857_106_811_133_404_017_207_799_481_956_647_721_349_092_596_212_439 ) end end
  ...
  describe '.valid_pubkey?' do
    ...
    it 'is true for compressed and uncompressed keys' do
      ...
      expect(Bitcoin.valid_pubkey?(key.pub_uncompressed)).to be true end

    it 'is false for invalid public keys' do
      expect(Bitcoin.valid_pubkey?(key.addr)).to be false
      expect(Bitcoin.valid_pubkey?(key.priv)).to be false end end
  ...
  describe '.bitcoin_mrkl / .bitcoin_hash' do
    ...
    it 'produces the expected values' do
      ...
      expect(Bitcoin.bitcoin_mrkl(a, b)).to eq(c) end end
  ...
  describe 'merkle trees' do
    let(:merkle_tree1) do
      ...
      %w[
        ...
        7dac2c5666815c17a3b36427de37bb9d2e2c5ccec3f8633eb91a4205cb4c10ff ] end
    let(:merkle_tree2) do
      %w[
        ...
        74f3a7df861d6a58957b84a3e425a8cf57e1e2e3a3def046dd200baeb8714f00 ] end
    let(:merkle_tree3) do
      %w[
        ...
        72e0e61880cc5fc9c7b5990c2d40b22eba783391b72807d2d5349fb55875c015 ] end
    let(:merkle_tree4) do
      %w[
        ...
        9ed561bb49c47648e0250cb074721d94cba84ed1e083f1e57c29eca78e36d73d ] end

    it 'produces the expected tree for a simple tree' do
      expect(Bitcoin.hash_mrkl_tree(merkle_tree1[0...2])).to eq(merkle_tree1) end
  ...
  describe '.decode_compact_bits / .encode_compact_bits' do
    ...
    it 'decodes nonce compact bits to bignum hex' do
      ...
      ).to eq('0000000065465700000000000000000000000000000000000000000000000000') end
    ...
    it 'encodes to the expected values' do
      ...
      expect(
        Bitcoin.encode_compact_bits(Bitcoin.decode_compact_bits(target3)) ).to eq(target3) end end
  ...
  describe '.block_hash' do
    ...
    let(:expected_block_hash0) do
      '000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f' end
    ...
    let(:expected_block_hash1) do
      '00000000839a8e6886ab5951d76f411475428afc90947ee320161bbf18eb6048' end
    ...
    let(:expected_block_hash169) do
      '000000002a22cfee1f2c846adbd12b3e183d4f97683f85dad08a79780a84bd55' end
    ...
    let(:expected_block_hash171) do
      '00000000c9ec538cab7f38ef9c67a95742f56ab07b0a37c5be6b02808dbfb4e0' end
    ...
    it 'produces the expected block hash' do
      ...
      ).to eq(expected_block_hash171) end end
  ...
  describe '.generate_key' do
    it 'generates an openssl-secp256k1 private/public keypair' do
      ...
      expect(Bitcoin.inspect_key(key)).to eq([private_key, public_key]) end end
    ...
    describe 'Bitcoin.open_key' do
      it 'opens key from private key and resolves public key' do
        50.times do
          ...
          expect(key.public_key_hex).to eq(public_key) end end end
      ...
      context 'mainnet' do
        ...
        let(:address_and_keys2) do
          %w[
            ...
            12b004fff7f4b69ef8650e767f18f11ede158148b425660723b9f9a66e61f747
            030b4c866585dd868a9d62348a9cd008d6a312937048fff31670e7e920cfc7a744 ] end
            ...
            16.times.each do |count|
              ...
              expect(
                Bitcoin.verify_message(
                  signature['address'],
                  'invalid-signature',
                  signature['message'] ) ).to be false
              expect(
                Bitcoin.verify_message(
                  signature['address'],
                  signature['signature'],
                  signature['message'] ) ).to be true end end end end end end
  ...
  it '.block_next_retarget' do
    ...
    expect(Bitcoin.block_next_retarget(189_504)).to eq(191_519) end

  it '.block_difficulty' do
    expect(Bitcoin.block_difficulty(436_835_377)).to eq('1751454.5353407') end
  ...
  it '.blockchain_total_btc' do
    ...
    block_heights.zip(expected_results).map do |height, expected_result|
      expect(Bitcoin.blockchain_total_btc(height)).to eq(expected_result) end end
  ...
  it '.block_creation_reward' do
    ...
    heights.zip(rewards).map do |height, reward|
      expect(Bitcoin.block_creation_reward(height)).to eq(reward) end end
  ...
  describe 'bitcoin base58 test vectors' do
    ...
    let(:valid_base58_keys) do
      JSON.parse(fixtures_file('base58_keys_valid.json')) end
    # Port of Bitcoin Core test vectors.
    ...
    let(:invalid_base58_keys) do
      JSON.parse(fixtures_file('base58_keys_invalid.json')) end
    ...
    it 'passes the valid keys cases' do
      valid_base58_keys.each do |test_case|
        ...
        expect(computed_address).to eq(address) end end
    ...
    it 'fails the invalid keys cases' do
      ...
      test_cases.each do |test_case|
        ...
        %i[bitcoin testnet3 regtest].each do |network_name|
          ...
          expect(Bitcoin.valid_address?(address)).to be false end end end end
  ...
  describe 'Bitcoin-Wiki - Common Standards - Hashes' do
    ...
    it 'double-SHA-256 encoding of string "hello"' do
      ...
      ).to eq('9595c9df90075148eb06860365df33584b75bff782a510c6cd4883a419833d50') end
    ...
    it 'RIPEMD-160 encoding of string "hello"' do
      ...
      ).to eq('b6a9c8c230722b7c748331a8b450f05566dc7d0f') end end end end
...
describe Bitcoin::Builder do
   ...
   result = build_block(target) do | b |
    ...
    b.tx do |t|
     ...
     t.output do |o|
      ...
      o.script do |s|
       s.type :address
       s.recipient keys[0].addr end end end end
    ...
    expect(tx.out[0].value).to eq(5_000_000_000) end
  ...
  it 'should build transactions with input and output signatures' do
    tx = build_tx do |t|
      t.input do |i|
        ...
        i.prev_out_index 0
        i.signature_key keys[0] end
      t.output do |o|
        o.value 123
        o.script do |s|
          s.type :address
          s.recipient keys[1].addr end end end

    expect(tx.in[0].prev_out.reverse_hth).to eq(block.tx[0].hash)
    expect(tx.in[0].prev_out_index).to eq(0)
    expect(Bitcoin::Script.new(tx.in[0].script_sig).chunks[1].unpack('H*')[0]).to eq(keys[0].pub)
    ...
    tx2 = build_tx do |t|
     ...
     t.output do |o|
       o.value 123
       o.script { |s| s.recipient keys[1].addr } end
     t.output do |o|
       o.to keys[1].addr
       o.value 321 end
     t.output { |o| o.to 'deadbeef', :op_return } end
    ...
    expect(Bitcoin::Script.new(tx2.out[0].pk_script).to_string
    ).to eq("OP_DUP OP_HASH160 #{keys[1].hash160} OP_EQUALVERIFY OP_CHECKSIG" )
    expect(tx2.out[2].value).to eq(0) end

  it 'should allow txin.prev_out as tx or hash' do
    prev_tx = block.tx[0]
    tx1 = build_tx do |t|
      t.input { |i| i.prev_out prev_tx, 0 } end
    tx2 = build_tx do |t|
      t.input { |i| i.prev_out prev_tx.hash, 0, prev_tx.out[0].pk_script } end
    expect(tx1.in[0]).to eq(tx2.in[0]) end
  ...
  it 'should provide tx#output shortcut' do
    ...
    expect(tx2).to eq(
      build_tx do |t|
        t.output do |o|
          o.value 123
          o.to keys[1].pub, :pubkey end end ) end
  ...
  it 'should provide txout#to shortcut' do
    ...
    tx2 = build_tx do |t|
      t.output do |o|
        o.value 123
        o.script { |s| s.recipient keys[1].addr } end end
    expect(tx1.out[0]).to eq(tx2.out[0]) end

  it 'should build unsigned transactions and add the signature hash' do
    tx = build_tx do |t|
      t.input do |i|
        i.prev_out block.tx[0]
        i.prev_out_index 0 end        # no signature key
      t.output do |o|
        o.value 123
        o.script { |s| s.recipient keys[1].addr } end end

    expect(tx).to be_a(Bitcoin::P::Tx)
    expect(tx.in[0].sig_hash).not_to be_nil end
  ...
  it 'should build unsigned multisig transactions and add the signature hash' do
    tx1 = build_tx do |t|
      ...
      t.output do |o|
        o.value 123
        o.to [2, *keys[0..2].map(&:pub)], :multisig end end
    ...
    tx2 = build_tx do |t|
      ...
      t.output do |o|
        o.value 123
        o.to keys[0].addr end end

    expect(tx2).to be_a(Bitcoin::P::Tx)
    expect(tx2.in[0].sig_hash).not_to be_nil end
  ...
  it 'should add change output and leave fee' do
    ...
    expect(
      Bitcoin::Script.new(tx.out.last.pk_script).get_address ).to eq(change_address)
    ...
    expect(tx.out.map(&:value).inject(:+)).to eq(50e8 - 10_000) end
  ...
  it 'should build op_return output' do
    ...
    expect(builder.txout.parsed_script.to_string).to eq('OP_RETURN deadbeef') end

  it 'should build op_return script' do
    result = script do |s|
      s.type :op_return
      s.recipient 'deadbeef' end
    expect(Bitcoin::Script.new(result).to_string).to eq('OP_RETURN deadbeef') end

  it 'should build address script' do
    key = Bitcoin::Key.generate
    result = script do |s|
      s.type :address
      s.recipient key.addr end

    expect(Bitcoin::Script.new(result).to_string)
      .to eq("OP_DUP OP_HASH160 #{Bitcoin.hash160_from_address(key.addr)} " \
             'OP_EQUALVERIFY OP_CHECKSIG') end

  it 'should build pubkey script' do
    key = Bitcoin::Key.generate
    result = script do |s|
      s.type :pubkey
      s.recipient key.pub end

    expect(Bitcoin::Script.new(result).to_string)
      .to eq("#{key.pub} OP_CHECKSIG") end

  it 'should build multisig script' do
    keys = Array.new(3) { Bitcoin::Key.generate }
    result = script do |s|
      s.type :multisig
      s.recipient 1, keys[0].pub, keys[1].pub end

    expect(Bitcoin::Script.new(result).to_string)
      .to eq("1 #{keys[0].pub} #{keys[1].pub} 2 OP_CHECKMULTISIG") end
  ...
  it 'should build and spend multisig output' do
    ...
    tx2 = build_tx do |t|
      t.input do |i|
        i.prev_out tx1, 0
        i.signature_key keys[0..1] end
      t.output do |o|
        o.value 123
        o.to keys[0].addr end end

    expect(tx2.verify_input_signature(0, tx1)).to be true end
  ...
  it 'should build and spend p2sh multisig output' do
    tx1 = build_tx do |t|
      ...
      t.output do |o|
        o.value 123
        o.to [2, *keys[0..2].map(&:pub)], :p2sh_multisig end end
    ...
    tx2 = build_tx do |t|
      t.input do |i|
        ...
        i.redeem_script tx1.out[0].redeem_script end

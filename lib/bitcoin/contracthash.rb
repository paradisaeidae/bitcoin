module Bitcoin
# Ruby port of https://github.com/Blockstream/contracthashtool DEPRECATE due to being sidechain implementaton.
module ContractHash
HMAC_DIGEST = OpenSSL::Digest.new('SHA256')
EC_GROUP = OpenSSL::PKey::EC::Group.new('secp256k1')
def self.hmac(pubkey, data)
 OpenSSL::HMAC.hexdigest(HMAC_DIGEST, pubkey, data) end
...
def self.compute_data(address_or_ascii, nonce_hex)
 ...
 if Bitcoin.valid_address?(address_or_ascii)
  ...
  address_type = 'TEXT'
  contract_bytes = address_or_ascii end
 [nonce.unpack('H*')[0], address_type + nonce + contract_bytes] end end end

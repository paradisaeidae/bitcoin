...
def encode(hrp, data)
 ...
 data.each do |i|
   ...
   buf << CHARSET[i] end
 6.times do
   chk = polymod_step(chk) end
 chk ^= 1
 6.times do |i|
   buf << CHARSET[(chk >> ((5 - i) * 5)) & 0x1f] end
 buf.pack('C*') end
...
def decode(input)
 ...
 hrp_len.times do |i|
  ...
  if ch >= 'a'.ord && ch <= 'z'.ord
    ...
    have_upper = true
    ch = (ch - 'A'.ord) + 'a'.ord end
  hrp << ch
  chk = polymod_step(chk) ^ (ch >> 5) end
 chk = polymod_step(chk)
 hrp_len.times do |i|
  chk = polymod_step(chk) ^ (input[i].ord & 0x1f) end
 data = []
 i = hrp_len + 1
 while i < input_len
  ch = input[i].ord
...
def convert_bits(chunks, from_bits:, to_bits:, pad:)
 ...
 chunks.each do |chunk|
  ...
  while bits >= to_bits
   bits -= to_bits
   output << ((buffer >> bits) & output_mask) end end
 output << ((buffer << (to_bits - bits)) & output_mask) if pad && bits > 0
 if !pad && (bits >= from_bits || ((buffer << (to_bits - bits)) & output_mask) != 0) then return nil end

...
def add_data(data)
 @nfunc.times.each do |fi|
  ...
  i = idx / 8
  @filter[i] = (@filter[i].ord | BIT_MASK[idx % 8]).chr end end
def contains?(data)
 @nfunc.times.all? do |fi|
  idx = calc_index(data, fi)
  i = idx / 8
  @filter[i].ord & BIT_MASK[idx % 8] != 0 end end
def add_address(address)
 add_data(Bitcoin.hash160_from_address(address).htb) end
def add_outpoint(prev_tx_hash, prev_output)
 add_data(prev_tx_hash.htb_reverse + [prev_output].pack('V')) end
private
#
...
def init_filter(elements, fp_rate)
 ...
 @nfunc = [1, [calc_k, MAX_HASH_FUNCS].min].max end
...
def calc_index(data, hash_index)
 ...
 h1 &= 0xffffffff
 h1 ^= h1 >> 16
 (h1 & 0xffffffff) % (@filter_size * 8) end end end
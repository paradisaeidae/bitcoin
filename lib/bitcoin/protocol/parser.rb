...
def initialize(handler = nil)
 ...
 @buf = ''
 @stats = { 'total_packets' => 0, 'total_bytes' => 0, 'total_errors' => 0 } end
...
def process_pkt(command, payload)
 ...
 else parse_error :unknown_packet, [command, payload.hth] end end
...
def parse_headers(payload)
 ...
 headers = Array.new(count) do
  ...
  b end
 @h.on_headers(headers) end

def parse_inv(payload, type = :put) # handles inv/getdata packets
 ...
 payload.each_byte.each_slice(36).with_index do |i, idx|
  ...
  when 2
   if type == :put
    ...
    else @h.on_inv_block(hash) end
   else @h.on_get_block(hash) end
  else parse_error :parse_inv, i.pack('C*') end end end
...
def parse_addr(payload)
  ...
  payload.each_byte.each_slice(30) do |i|
   ...
   @h.on_addr(Addr.new(i.pack('C*')))
    ...
    parse_error(:addr, i.pack('C*')) end end end

def parse_verack
 if @h.respond_to?(:on_verack)
  @h.on_verack
 else @h.respond_to?(:on_handshake_complete) ? @h.on_handshake_complete : nil end end

def parse_version(payload)
 @version = Bitcoin::Protocol::Version.parse(payload)
 @h.on_version(@version) end

def parse_alert(payload); end # nop (https://github.com/lian/bitcoin-ruby/issues/268)
...
def parse_getblocks(payload)
 ...
 [version, hashes, stop_hash] end

def handle_mempool_request(*_) # https://en.bitcoin.it/wiki/BIP_0035
 ...
 @h.on_mempool if @h.respond_to?(:on_mempool) end
 ...
 if magic == head_magic
  if Digest::SHA256.digest(Digest::SHA256.digest(payload))[0...4] != checksum
   if (length < 50_000) && (payload.size < length)
    ...
    handle_stream_error(:close, 'checksum mismatch') end
   return
  end
...
def handle_stream_error(type, msg)
 ...
 else puts [type, msg].inspect end end

def parse_error(*err)
 @stats['total_errors'] += 1
 return unless @h.respond_to?(:on_error)
 @h.on_error(*err) end end end end
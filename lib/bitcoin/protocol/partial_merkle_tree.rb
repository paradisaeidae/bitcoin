...
def tx_hashes
 @leaves.reject { |n| n.value.nil? }.map(&:value) end

def build_tree
 lay = @leaves = Array.new(@total_txs) { Node.new(nil, nil, nil) }
 while lay.size > 1
  lay = lay.each_slice(2).map do |left, right|
   Node.new(nil, left, right) end end
 lay[0] end

def current_flag
 (@flags[@visit_idx / 8].ord & BIT_MASK[@visit_idx % 8]).zero? end
...
def assign_value(node = root)
 ...
 if node.left
  @visit_idx += 1
  assign_value(node.left) end
 if node.right
  @visit_idx += 1
  assign_value(node.right) end

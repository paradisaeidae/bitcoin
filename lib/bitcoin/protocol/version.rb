module Bitcoin
module Protocol  # https://en.bitcoin.it/wiki/Protocol_specification#version
class Version    # services bit constants
NODE_NONE = 0
NODE_NETWORK = (1 << 0)
attr_reader :fields
def initialize(opts = {})
 @fields = {
  ...
  last_block: 0, # 188617
  relay: true # BIP0037
 }.merge(opts.reject { |_k, v| v.nil? }) end
...
def to_payload
 [@fields.values_at(:version, :services, :time).pack('VQQ'),
  ...
  Protocol.pack_boolean(@fields[:relay])].join end

def to_pkt
 Bitcoin::Protocol.pkt('version', to_payload) end
...
def pack_address_field(addr_str)
 ...
 port = sockaddr[2...4]
 host = sockaddr[4...8]
 [[1].pack('Q'), "\x00" * 10, "\xFF\xFF", host, port].join end
...
def unpack_relay_field(version, payload)
 version >= 70_001 && payload ? Protocol.unpack_boolean(payload) : [true, nil] end

def uptime
 @fields[:time] - Time.now.tv_sec end
...
def self.parse(payload)
 new.parse(payload) end end end end
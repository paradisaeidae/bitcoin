...
def prev_block_hex
 @prev_block_hex ||= @prev_block_hash.reverse.unpack('H*')[0] end

def initialize(data = nil) # create block from raw binary +data+
 @tx = []
 @payload = nil
 parse_data_from_io(data) if data end

def parse_data(data) # parse raw binary data
 buf = parse_data_from_io(data)
 buf.eof? ? true : buf.read end

def parse_data_from_io(buf, header_only = false) # parse raw binary data
 ...
 if !Bitcoin.network[:auxpow_chain_id].nil? && (@ver & BLOCK_VERSION_AUXPOW) > 0
  ...
  @aux_pow.parse_data_from_io(buf) end
...
def parse_filtered_header(buf)
 ...
 hashes = []
 nhashes.times do hashes << buf.read(256 / 8) end
...
def recalc_block_hash # recalculate the block hash
 @hash = Bitcoin.block_hash(
  @prev_block_hash.reverse_hth, @mrkl_root.reverse_hth, @time, @bits, @nonce, @ver ) end
...
def recalc_mrkl_root
  @mrkl_root = if partial_merkle_tree
   ...
   else Bitcoin.hash_mrkl_tree(@tx.map(&:hash)).last.htb_reverse end end

def verify_mrkl_root # verify mrkl tree
 if partial_merkle_tree
  partial_merkle_tree.valid_tree?(@mrkl_root.reverse_hth)
 else @mrkl_root.reverse_hth == Bitcoin.hash_mrkl_tree(@tx.map(&:hash)).last end end

def tx_hashes
 if partial_merkle_tree
  partial_merkle_tree.tx_hashes
 else @tx.map(&:hash) end end
...
def header_info
  [ @ver, @prev_block_hash.reverse_hth, @mrkl_root.reverse_hth,
    Time.at(@time), @bits, @nonce, @tx.size, @payload.size ] end
...
def hextarget
 Bitcoin.decode_compact_bits(@bits) end

def decimaltarget
 Bitcoin.decode_compact_bits(@bits).to_i(16) end

def difficulty
 Bitcoin.block_difficulty(@bits) end
...
def bip34_block_height(height = nil)
 ...
 if height # generate height binary
  ...
  coinbase[1..coinbase[0].ord].ljust(4, "\x00").unpack('V').first end
...
def header_to_json(options = { space: '' }) # convert header to json representation.
 h = to_hash
 %w[tx mrkl_tree].each { |k| h.delete(k) }
 JSON.pretty_generate(h, options) end

def block_header # block header binary output
 [ @ver, @prev_block_hash, @mrkl_root, @time, @bits, @nonce, Protocol.pack_var_int(0)
 ].pack('Va32a32VVVa*') end
...
def block_work
 ...
 (2**256) / (target + 1) end end end end
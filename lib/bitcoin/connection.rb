...
def on_get_transaction(hash)
 p ['get transaction', hash.hth] end

def on_get_block(hash)
 p ['get block', hash.hth] end

def on_addr(addr)
 p ['addr', addr, addr.alive?] end

def on_tx(tx)
 p ['tx', tx.hash] end

def on_block(block)
 p ['block', block.hash] end
 # p block.payload.each_byte.map{|i| "%02x" % [i] }.join(" ")
 # puts block.to_json

def on_version(version)
 p [@sockaddr, 'version', version, version.time - Time.now.to_i]
 send_data(Protocol.verack_pkt) end
...
def initialize(host, port, connections)
 ...
 @parser = Bitcoin::Protocol::Parser.new(self) end

def post_init
 p ['connected', @sockaddr]
 EM.schedule { on_handshake_begin } end
...
def unbind
 ...
 self.class.connect_random_from_dns(@connections) end

def self.connect(host, port, connections)
 EM.connect(host, port, self, host, port, connections) end

def self.connect_random_from_dns(connections)
 seeds = Bitcoin.network[:dns_seeds]
 if seeds.empty? then raise 'No DNS seeds available. Provide IP, configure seeds, or use different network.' end
 host = Resolv::DNS.new.getaddresses(seeds.sample).map(&:to_s).sample
 connect(host, Bitcoin.network[:default_port], connections) end end end

...
else ffi_lib ['libssl.so.1.1.0', 'libssl.so.1.1', 'libssl.so.1.0.0', 'libssl.so.10', 'ssl' ] end
...
begin
 ...
 def self.version
  if self.respond_to?(:OpenSSL_version_num) then OpenSSL_version_num()
  else SSLeay() end end
 if version >= VERSION_1_1_0_NUM
  ...
  attach_function :SSL_load_error_strings, [], :void end
 ...
 def self.regenerate_key(private_key)
  ...
  if priv_hex != private_key_hex
    raise 'regenerated wrong private_key, raise here before generating a faulty public_key too!' end
 ...
 def self.init_ffi_ssl
  ...
  if version >= VERSION_1_1_0_NUM
   OPENSSL_init_ssl( OPENSSL_INIT_LOAD_SSL_STRINGS | OPENSSL_INIT_ENGINE_ALL_BUILTIN, nil )
  else
   SSL_library_init()
   ERR_load_crypto_strings()
   SSL_load_error_strings() end
  RAND_poll()

# encoding: ascii-8bit
module Bitcoin; module OpenSSL_EC # rubocop:disable Naming/ClassAndModuleCamelCase
extend FFI::Library
if FFI::Platform.windows? then ffi_lib 'libeay32', 'ssleay32'
else ffi_lib ['libssl.so.3', 'ssl' ] end
NID_secp256k1 = 714 # rubocop:disable Naming/ConstantName
POINT_CONVERSION_COMPRESSED   = 2
POINT_CONVERSION_UNCOMPRESSED = 4
# OpenSSL 3.0.2 version as a numerical version value as defined in:
# https://www.openssl.org/docs/man1.1.0/man3/OpenSSL_version.html
VERSION_3_0_2_NUM = 0x30002000
OPENSSL_INIT_LOAD_SSL_STRINGS = 0x00200000 # Same as 1.1 : https://github.com/openssl/openssl/blob/openssl-3.0/include/openssl/ssl.h.in

begin # Determine what version of OpenSSL we are interacting with.
 attach_function :OpenSSL_version_num, [], :ulong
rescue FFI::NotFoundError
 puts 'FFI::NotFoundError'
 attach_function :SSLeay, [], :long end

 # Returns the version of SSL present version number as an integer.
 # The first two digits contain the major release number, the third and fourth digits the minor release number,
 # and the fifth and sixth digits the fix release number. For OpenSSL, the seventh and eight digits contain the patch release number
 # and the final digit is 0 for development, 1 to e for betas 1 to 14, or f for release. 
 def self.version
  if self.respond_to?(:OpenSSL_version_num) then
   vers = OpenSSL_version_num()
  else p'Using SSLeay for version'
   SSLeay() end
  major = (vers >> 28) & 0xFF
  minor = (vers >> 20) & 0xFF
  patch = (vers >> 12) & 0xFF
  if major < 3 then raise 'Cannot find Openssl major version 3' end end
 version()
 ...
 def self.regenerate_key(private_key)
  ...
  key = ::MoneyTree::PrivateKey.new(key: private_key)
  debugger
  key.public_key = group.generator.mul(key.private_key)

  priv_hex = key.private_key.to_bn.to_s(16).downcase.rjust(64, '0')
  if priv_hex != private_key_hex then raise 'regenerated wrong private_key, raise here before generating a faulty public_key too!' end
  [priv_hex, key.public_key.to_bn.to_s(16).downcase] end
 ...
 # algorithm in SEC1v2 section 4.1.6.________________
 ...
 # you find one that outputs the key you are expecting.________
 # If this method returns nil, it means recovery was not possible and rec_id
 # should be iterated.________________
 ...
 # one you expect, you try again with the next rec_id.___________
 ...
 def self.sign_compact(hash, private_key, public_key_hex = nil, pubkey_compressed = nil)
  ...
  r, s = signature.get_array_of_pointer(0, 2).map do |i|
   BN_bn2bin(i, buf)
   buf.read_string(BN_num_bytes(i)).rjust(32, "\x00") end
  rec_id = nil
 ...
 def self.recover_compact(hash, signature)
  ...
  recover_public_key_from_signature(msg32.read_string(32), signature, version - 27, compressed) end
 ...
 # Regenerate a DER-encoded signature such that the S-value complies with BIP62.
...
=end
 ...
 def self.init_ffi_ssl
  ...
  OPENSSL_init_ssl( OPENSSL_INIT_LOAD_SSL_STRINGS | VERSION_3_0_2_NUM, nil )
  RAND_poll()
  @ssl_loaded = true end end end
...
# DEPR3 OPENSSL_INIT_ENGINE_RDRAND    = 0x00000200
# DEPR3 OPENSSL_INIT_ENGINE_DYNAMIC   = 0x00000400
# DEPR3 OPENSSL_INIT_ENGINE_CRYPTODEV = 0x00001000
# DEPR3 OPENSSL_INIT_ENGINE_CAPI      = 0x00002000
# DEPR3 OPENSSL_INIT_ENGINE_PADLOCK   = 0x00004000
# DEPR3 OPENSSL_INIT_ENGINE_ALL_BUILTIN = ( OPENSSL_INIT_ENGINE_RDRAND    | OPENSSL_INIT_ENGINE_DYNAMIC |

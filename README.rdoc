== This was forked from bitcoin-ruby now dis-forked.
== It has been reduced to the fundamental bitcoin state.
== Removed references to litecoin, dogecoin, namecoin, Bech32
This is a ruby library for interacting with the bitcoin protocol/network.
As per the adjustments to return to:
https://github.com/bitcoin-sv-specs/protocol/blob/master/updates/genesis-spec.md
https://www.bsvblockchain.org/releases/changes-for-the-genesis-upgrade
...
Some adjustments:
* Script limit increased to 500k
* Removed references to fork_id
* Removed all P2SH code, references and tests.
...
== Installation
 ...
 gem install bitcoin
 # OR
 git clone https://github.com/paradisaeidae/bitcoin.git; cd bitcoin; bundle install
...
== libsecp256k1
A compiled binary is located withing the gem. This is a linux binary.
In the folder as such: libsecp256k1/lib/libsecp256k1.so
Location of this binary is explicit currently.

* +rspec+ to run the specs

If you would like to install using Bundler, put it in your Gemfile and run bundle install
 gem 'bitcoin', git: 'https://github.com/paradisaeidae/bitcoin', branch: 'master', require: 'bitcoin'
...
If you want to control the Bitcoin::Script yourself, you can do so
 ...
 result = script.run do |pubkey, sig, hash_type|
  hash = tx1.signature_hash_for_inputs(0, nil, txout.pk_script)
  Bitcoin.verify_signature(hash, sig, pubkey.unpack("H*")[0]) end
 result #=> true

=== Create Transactions

You need to know the onchained (previous) outpoint you want to spend (tx hash and output index),
as well as the private key for the address required to sign for it.
 ...
 # the onchained transaction that has an output to your address
 ...
 prev_tx = Bitcoin::P::Tx.from_json(open("http://TBA/tx/#{prev_hash}.json"))
 
 # the key needed to sign an inpoint that spends the onchained outpoint
 ...
 new_tx = build_tx do |t|
   ...
   t.output do |o|
    o.value 50000000 # 0.5 BSV in satoshis
    o.script {|s| s.recipient "mugwYJ1sKyr8EDDgXtoh8sdDQuNWKYNf88" } end
   ...
   t.output do |o|
    o.value 49000000 # 0.49 BSV, leave 0.01 BSV as fee
    o.script {|s| s.recipient key.addr } end end
 
 # examine your transaction. you can relay it through https://TBA
...
should run the specs individually.

== Development
  
Previous authors are acknowledged. Any comments or interactions are welcome.  

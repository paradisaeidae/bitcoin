...
RSpec::Core::RakeTask.new(:rspec) do |t| t.rspec_opts = '--exclude spec/unit/integrations/*' end

RSpec::Core::RakeTask.new(:coin_spec, :coin) do |t, args|
  t.rspec_opts = "--pattern spec/unit/integrations/#{args[:coin]}_spec.rb" end
...
task :build_libsecp256k1, [:force] do |_, args|
 ...
 sh "rm -rf #{LIBSECP256K1_PATH}" end
